...
- **Password reset does not exist yet** — there is no `/auth/forgot-password` route and the UI says so. It needs a transactional email provider (e.g. Resend) first; until then a forgotten password means a lost account. This is the largest remaining known gap. See `docs/AUDIT.md` F5.
